...
### 🔘设置选项
<details>
<summary>- X.0 (mpv.net v6.0)版界面</summary>
...
⭕X.0 (mpv.net v6.0)版
- 在`mpv.conf编辑器`的`高级`列表里，将其路径填入`vf`（视频滤镜）或`glsl-shaders`（着色器）的选项框中
  > 示例：   
  > - 在`vf`选项框中填入
  > ```editorconfig
  ...
  > ```
⭕202X.0 (mpv.net v7.0+)版
- 在`portable_config`文件夹中的`mpv.conf`，写入vf（视频滤镜）或glsl-shaders（着色器）的路径
  > 示例：
  > ```editorconfig
